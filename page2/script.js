(() => {
  ...
  const SidebarManager = {
    ...
    async fetchAndRenderRooms(){
      ...
      } catch (err) { this.roomList.innerHTML = `<li style="padding:12px;color:#ff8080;font-size:0.8rem;text-align:center;">${err.message}</li>`;}
    },
  ...
  // 페이지 로드되면 여기서 초기 세팅 진행
  document.addEventListener("DOMContentLoaded", () => {
    const urlParams = new URLSearchParams(window.location.search);
    const token = urlParams.get('access_token');

    // URL에 토큰 있으면 로컬 스토리지에 저장하고 주소창 깔끔하게 지워줌
    if (token) {
